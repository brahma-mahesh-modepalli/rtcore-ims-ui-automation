...
test.describe("RCSP-45 - Inventory Baselines on IMS 1034 and Non-IMS 1025", () => {
  test("Verify whether ON HAND and FIFO COST for LARGE BUNS 11201 can be recorded on WB Unit 1034 and WB Unit 1025", { tag: ['@functional'] }, async ({ page }) => {
    const { transfersPage } = await loginAsAdmin(page);
    const common = getCommonData();
    const data = getCaseData<{
      store1034: string;
...
test.describe("RCSP-45 - New Transfer Store Dropdown Rules (IMS From / IMS+Non-IMS To)", () => {
  test("Verify whether From Store is IMS-only and To Store lists IMS and Non-IMS stores", { tag: ['@functional'] }, async ({ page }) => {
    const { transfersPage } = await loginAsAdmin(page);
    const data = getCaseData<{
      fromStore: string;
      imsToExample: string;
...
test.describe("RCSP-45 - IMS to Non-IMS Submit → Completed + source ON HAND -1", () => {
  test("Verify whether submitting IMS→Non-IMS transfer From 1034 To 698 sets Completed immediately and decrements ON HAND", { tag: ['@functional'] }, async ({ page }) => {
    const { transfersPage } = await loginAsAdmin(page);
    const common = getCommonData();
    const data = getCaseData<{
      fromStore: string;
...
test.describe("RCSP-45 - IMS to IMS Submit → Pending (contrast)", () => {
  test("Verify whether submitting IMS→IMS transfer From 1034 To 1008 results in Pending status", { tag: ['@functional'] }, async ({ page }) => {
    const { transfersPage } = await loginAsAdmin(page);
    const common = getCommonData();
    const data = getCaseData<{
      fromStore: string;
...
test.describe("RCSP-45 - Draft then Submit IMS to Non-IMS → Completed", () => {
  test("Verify whether SAVE CHANGES creates Draft and SUBMIT from Draft Details yields Completed for Non-IMS destination", { tag: ['@functional'] }, async ({ page }) => {
    const { transfersPage } = await loginAsAdmin(page);
    const common = getCommonData();
    const data = getCaseData<{
      fromStore: string;
...
test.describe("RCSP-45 - New Transfer mandatory validations", () => {
  test("Verify whether SUBMIT is blocked when From Store, To Store, Reason, or Line Items are missing", { tag: ['@regression'] }, async ({ page }) => {
    const { transfersPage } = await loginAsAdmin(page);
    const common = getCommonData();
    const data = getCaseData<{
      fromStore: string;
...
test.describe("RCSP-45 - Invalid store / quantity rules", () => {
  test("Verify whether same From/To, Non-IMS From Store, and invalid quantities are blocked", { tag: ['@regression'] }, async ({ page }) => {
    const { transfersPage } = await loginAsAdmin(page);
    const common = getCommonData();
    const data = getCaseData<{
      fromStore: string;
...
test.describe("RCSP-45 - New Transfer UI edge behaviors", () => {
  test("Verify whether SAVE/SUBMIT stay disabled until complete and To Store search filters correctly", { tag: ['@regression'] }, async ({ page }) => {
    const { transfersPage } = await loginAsAdmin(page);
    const data = getCaseData<{
...
test.describe("RCSP-45 - Cancel Draft & lock Completed", () => {
  test("Verify whether Draft IMS→Non-IMS can be Cancelled without stock deduction and Completed is read-only", { tag: ['@functional'] }, async ({ page }) => {
    const { transfersPage } = await loginAsAdmin(page);
    const common = getCommonData();
    const data = getCaseData<{
      fromStore: string;
...
test.describe("RCSP-45 - End-to-End full IMS to Non-IMS flow", () => {
  test("Verify whether hierarchy → baselines → New Transfer IMS→Non-IMS → SUBMIT results in Completed and source ON HAND reduction", { tag: ['@functional'] }, async ({ page }) => {
    const { transfersPage } = await loginAsAdmin(page);
    const common = getCommonData();
    const data = getCaseData<{
      fromStore: string;
...
test.describe("RCSP-45 - Status matrix IMS vs Non-IMS To Store", () => {
  test("Verify whether IMS→Non-IMS submits Completed and IMS→IMS submits Pending", { tag: ['@functional'] }, async ({ page }) => {
    const { transfersPage } = await loginAsAdmin(page);
    const common = getCommonData();
    const data = getCaseData<{
