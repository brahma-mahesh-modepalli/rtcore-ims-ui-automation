/**
 * RCSP-283 – Inventory - Vendor Items: DB vs UI Price validation
 * Test case IDs: TC_RCSP-283_01 … TC_RCSP-283_09.
 *
...
const RCSP_283_FILE_NAME = 'RCSP-283';
const RCSP_283_SCENARIO_ID = 'RCSP-283';

const TC = {
  matchAllVendor2: 'TC_RCSP-283_01',
  noMissingVendor2: 'TC_RCSP-283_02',
  latestMatchVendor3: 'TC_RCSP-283_03',
  latestSelectedOnDuplicate: 'TC_RCSP-283_04',
  excludeNullDates: 'TC_RCSP-283_05',
  priceVendor2: 'TC_RCSP-283_06',
  searchReturnsCorrectSku: 'TC_RCSP-283_07',
  priceLatestVendor3: 'TC_RCSP-283_08',
  allSkusLatestVendor3: 'TC_RCSP-283_09',
} as const;

function getCaseData<T>(testCaseId: string): T {
  return getScenarioTestCaseData<T>(
    RCSP_283_FILE_NAME,
    RCSP_283_SCENARIO_ID,
    testCaseId,
  );
}
...
test.describe('RCSP-283 - Vendor Items DB vs UI Price validation', () => {
  ...
  test('TC_RCSP-283_02 – All database SKUs for Vendor ID 2 can be searched and validated in the UI', { tag: ['@functional'] }, async ({
    testData,
  }) => {
  ...
  test('TC_RCSP-283_03 – Price for searched SKU matches the latest DB unit_cost for Vendor ID 3', { tag: ['@functional'] }, async ({
    testData,
  }) => {
  ...
  test('TC_RCSP-283_04 – Latest vendor item Price is displayed when duplicates exist for Vendor ID 3', { tag: ['@regression'] }, async ({
    testData,
  }) => {
  ...
  test('TC_RCSP-283_05 – Records with null available_from/expires_at are excluded from Vendor ID 3 Price validation', { tag: ['@regression'] }, async ({
    testData,
  }) => {
  ...
  test('TC_RCSP-283_06 – Price displayed for every Vendor ID 2 SKU matches the corresponding DB unit_cost', { tag: ['@functional'] }, async ({
    testData,
  }) => {
  ...
  test('TC_RCSP-283_07 – Vendor ID 2 SKU search returns the correct vendor item before Price validation', { tag: ['@functional'] }, async ({
    testData,
  }) => {
  ...
  test('TC_RCSP-283_08 – Price displayed for the latest Vendor ID 3 record matches the DB unit_cost', { tag: ['@functional'] }, async ({
    testData,
  }) => {
  ...
  test('TC_RCSP-283_09 – All Vendor ID 3 SKUs are searched and their Price validated against the latest DB unit_cost', { tag: ['@functional'] }, async ({
    testData,
  }) => {
